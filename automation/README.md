...
- 트렌드 API 대신 `data/questions-pool.json`에 미리 넣어둔 한국어 질문 풀에서 매번 하나를 골라, Claude API로 **영어 한 줄 자막**으로 번역/다듬고 분위기에 맞는 배경음악 장르를 태깅합니다 (1단계).
- 각 단계는 `npm run step1:question` 처럼 독립 실행 가능하며, `data/`에 결과를 JSON으로 저장해 다음 단계가 읽습니다. 전체는 `npm run pipeline`으로 순서대로 실행합니다.
- 영상에 들어가는 자막(질문/카드별 캐치프레이즈/마지막 CTA)은 모두 **영어**로 생성됩니다. (질문 풀 원본 데이터는 한국어로 유지)
